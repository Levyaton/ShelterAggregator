...
public class ScraperService {
    ...
    private static final String LEGAL_DISCLAIMER = "Žádná část těchto stránek, včetně fotografií, nesmí být kopírována a rozmnožována za účelem rozšiřování v jakékoliv formě či jakýmkoliv způsobem bez písemného souhlasu provozovatele portálu.";
    @Autowired
    private TaskExecutor scraperExecutor;

    @Scheduled(fixedDelayString = "${scrape.interval:3600000}")
    public void dispatchPages() throws Exception {
        Document doc = Jsoup.connect(BASE + SEARCH_DOGS)
    ...
    private void scrapeDogListPage(int offset) {
        try {
            Document pageDoc = Jsoup.connect(BASE + "?start=" + offset)
                    .userAgent("MyScraper/1.0")
                    .timeout(10_000)
                    .get();

            pageDoc.select("div.object-item ")
                    .stream()
                    .map(el -> el.attr("href"))
                    .forEach(this::submitDogDetailJob);

        } catch (IOException e) {
            // log & retry policy if desired
        }
    }
    ...
    private static DogDto scrapeDogDetail(String url) {
     ...
     static Pair<Integer, Integer> getWeight(Element detail) {
        String weightText = detail.selectFirst("#dogdetail #dd-left table tbody tr td:contains(Velikost)").text().split(" ")[1].strip();
        switch (weightText){
            case "Malý" -> {
                return Pair.of(DogSize.SMALL.getFrom(), DogSize.MEDIUM.getFrom()); // small dogs
            }
            case "Střední" -> {
                return Pair.of(DogSize.MEDIUM.getFrom(), DogSize.LARGE.getFrom()); // medium dogs
            }

            case "Velký" -> {
                return Pair.of(DogSize.LARGE.getFrom(), 55); // large dogs
            }
            default -> {
                return Pair.of(0, 55);
            }
     ...
     static String getAddress(Element detail) {
        String address = detail.selectFirst("tr:contains(umístění) td").childNodes().getFirst().attributes().attribute("href").getValue();
        return BASE + address;
    }
     ...
     static Float getAge(Element detail) {
        Element ageElement = detail.selectFirst("td:contains(věk)");
         if (ageElement == null) {
            return null; // default age if not found
         }
         var age = ageElement.text();
        var ageStringParts = age.split(" ");
        if (ageStringParts[2].equals("měsíce")){
            return Float.parseFloat(ageStringParts[1]) / 12; // convert months to years
        } else {
            return Float.parseFloat(ageStringParts[1]); // already in years
        }
    }
